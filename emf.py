...
def mf_handler(S=[],n=0,V=[],F=[],P={'0':[]},H=[]):
    ...
    return output_table
"""
choice = input("Do you want to read from file (1) or input via prompt (2)? Enter 1 or 2: ")
...
if choice == '1':
    ...
    with open(filename, "r") as file:
        input_lines = [file.readline().strip() for _ in range(6)]
elif choice == '2':
    prompts = [
        "Enter line for S (e.g., S = [...]): ",
        "Enter line for P (e.g., P = {...}): ",
        "Enter line for V (e.g., V = [...]): ",
        "Enter line for F (e.g., F = [...]): ",
        "Enter line for n (e.g., n = 3): ",
        "Enter line for H (e.g., H = [...]): "
    ]
...
# Now write everything into the new file
with open("run_emf_query.py", "w") as out_file:
    out_file.write(main_stuff)
    out_file.write("\n\n")
    for line in input_lines:
        out_file.write(line + "\n")
    out_file.write("\n")
    out_file.write("dict_return = mf_handler(S=S,n=n,V=V, F=F,P=P,H=H)\n")

    out_file.write("print(dict_return)\n")
    out_file.write('''with open("table_output.txt", "w") as file:\n    file.write(dict_return)\n''')

print("File 'run_emf_query.py' has been created successfully.")